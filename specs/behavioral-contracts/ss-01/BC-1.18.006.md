...
10. **NEW (cluster-2 LOCAL adversary pass-3, O-C2-P3-001, ADVISORY) — Postcondition 7 catch point
    (i) (`reconcile_post_write_replace_all_overcap`, PostToolUse) is safe to run WITHOUT a
    self-heal pre-pass, asymmetrically with the PreToolUse Flat arm (which runs
    `run_self_heal_if_plausible` first) — BY CONSTRUCTION, not by coincidence, and this asymmetry
    MUST NOT be silently removed by a future call-order change.** Catch point (i) calls
    `execute_roll` directly, with no preceding self-heal pass, because BOTH kinds of orphan a
    self-heal pass could otherwise need to clear first are already harmless to catch point (i)'s
    own correctness:
    - **An `E-SHD-007` orphan** (a prior roll's canonical-truncate succeeded but its index-publish
      did not) leaves the canonical file EMPTY. Catch point (i) runs immediately after the CURRENT
      `replace_all` write has already been applied on top of whatever the canonical file held at
      that moment; its own `stat()` reads the CURRENT, post-write size. An unreconciled `E-SHD-007`
      orphan does not change what that `stat()` observes — the canonical file's actual byte count is
      unaffected by whether its OWN prior seal is indexed yet — so catch point (i)'s own
      under/over-cap determination is unaffected, and it neither needs nor benefits from clearing
      the orphan first. (A subsequent dispatch's own trigger evaluation may still want the index
      reconciled for OTHER reasons — e.g. accurate `seq` allocation for a NEW roll — but that
      reconciliation is not a PRECONDITION for catch point (i)'s own correctness on the CURRENT
      call, since catch point (i) does not consult the index to decide whether to roll.)
    - **An `E-SHD-006` orphan** (a prior roll's seal succeeded but its truncate did not) leaves the
      canonical file holding a byte-for-byte DUPLICATE of already-sealed content. If catch point
      (i)'s own roll fires against that canonical state, it re-seals the SAME bytes into a NEW
      `<stem>.<seq+1:04>.md` file — a duplicate shard, not a data-loss or unique-content-loss event;
      no history is destroyed, only harmlessly re-copied under a new `seq`. This is wasteful (an
      extra shard file, an extra index entry) but not unsafe, and is self-correcting: the next
      self-heal pass (triggered by any subsequent dispatch's own Postcondition 1 entry, or an
      operator-invoked reconciliation sweep) still recognizes and reconciles the original
      `E-SHD-006` state independently, since catch point (i)'s duplicate seal does not consume or
      alter the orphaned sealed file self-heal was going to find.
    This asymmetry with the PreToolUse Flat arm (BC-1.18.005 Precondition 1, which DOES run
    `run_self_heal_if_plausible` first) is therefore intentional and safe, not an oversight to be
    "fixed" by adding a matching self-heal pre-pass to catch point (i) — doing so would add cost
    (an extra reconciliation scan on every `replace_all` post-write check) with no corresponding
    safety benefit, since neither orphan class can corrupt catch point (i)'s own outcome. A future
    change to catch point (i)'s call order or to either orphan class's recovery semantics MUST
    re-verify both bullets above before removing this asymmetry or asserting it is no longer safe.
...
- `crates/factory-dispatcher/src/shard_manager.rs` `execute_roll` (NEW, cluster-2 LOCAL adversary pass-3, F-C2-P3-001) — the `Ok(None)` empty-canonical short-circuit (Postcondition 1) and its distinct `build_empty_roll_retry_block_reason` message-construction function (Postcondition 2's "Empty-canonical retry template"), invoked from the SAME trigger-fire branch that constructs the unified rotate-and-retry template, selecting between the two per Invariant 4's adjudicated two-case scoping
...
- VP-NNN (pending, F2 spec-evolution, replace_all closure; EXTENDED cluster-2 LOCAL adversary pass-1, F-C2-P1-001/002; EXTENDED cluster-2 LOCAL adversary pass-2, F-C2-P2-003/004) — Postcondition 7's bounded post-write reconciliation for under-projected `replace_all: true` writes (catch point (i)/(ii) `stat()`-based reconciliation, retroactive four-step roll reuse, `sealed_retroactively` audit flag, bounded-window guarantee); EXTENDED to cover catch point (ii)'s per-tool `stat()` cost split and `Write`-path data-loss closure (EC-017, Invariant 7's sibling scope) and the `sealed_retroactively` recovery-by-inference rule for `E-SHD-007` self-heal (EC-018, Invariant 7). **EXTENDED (pass-2):** catch point (ii)'s uniform fail-loud backstop-probe `stat()`-failure disposition — `NotFound` proceeds, every other error returns `HookResult::Error`/`E-SHD-008` without applying the `Write` (EC-019, Invariant 8); Invariant 7's Stable-Cap Precondition governing the bounded, accepted, bidirectional `sealed_retroactively` mislabel risk under a `shard_cap_bytes` re-calibration spanning a shard's seal-to-reconciliation window (EC-020). Allocation routed to architect/formal-verifier at Phase F6 targeted-hardening, mirroring BC-1.18.005 EC-017's own VP-owed-to-F6 precedent (S2502-CLUSTER1-PASS5 STATE.md Drift Item) — NOT self-allocated by product-owner in this burst.
