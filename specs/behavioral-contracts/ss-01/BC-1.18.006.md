...
1. **CORRECTED (fix-burst pass-2, F-P2-003/F-P2-004, HIGH/MEDIUM) — the roll sequence is a
   ...
   path, since sealed filenames are never read by shard-unaware code) — **write-once:
   `publish_sealed_shard` MUST refuse to complete, and MUST NOT overwrite, if the destination path
   already exists on disk (CORRECTED, cluster-2 LOCAL adversary pass-4, F-C2-P4-001/002; see
   Postcondition 8 and the corrected Invariant 10)**; (c) **atomically REPLACE
   ...
   **Partial-failure postconditions — one named `E-SHD-NNN` code per crash point (ADR-051
   ...
   - **Step (c) fails after step (b) succeeded (NEW `E-SHD-006`):** the sealed shard now durably
     ...
     identical content. **This recovery premise — that the SAME artifact's next dispatch attempt
   ...
   **Mechanism — two redundant catch points, bounding the observable-over-cap window to at most one
   ...
   - **(ii) Next-dispatch backstop (defense-in-depth, covers a crash between apply and (i)).**
     ...
     **Backstop probe `stat()`-failure disposition — fail LOUD on any non-`NotFound` error, never
     ...
     `E-SHD-008`, "backstop probe stat failure") WITHOUT applying the `Write`, identically to how
...
8. **NEW (cluster-2 LOCAL adversary pass-4, F-C2-P4-001/002, MAJOR/MINOR) — `publish_sealed_shard`
   is WRITE-ONCE: it MUST refuse to overwrite an existing destination, and a collision is a loud
   error, never a silent data-loss overwrite.** A sealed shard, once published, is immutable content
   — Postcondition 3's guarantee that a prospective seal's `bytes_at_seal <= shard_cap_bytes` (or,
   for a retroactive seal, Postcondition 7's documented exception) and Invariant 3's "canonical
   filename never moves" guarantee both assume the sealed file at a given `seq` never changes after
   publication. Before every `write_atomic` `rename(temp, <stem>.<seq:04>.md)` call this BC's gate
   issues to publish a sealed shard (Postcondition 1 step (b), and both of Postcondition 7's catch
   points (i)/(ii) when they execute the same step retroactively), the gate MUST first verify the
   destination path does not already exist. If it does, the gate MUST NOT perform the rename — it
   returns `HookResult::Error` (NEW `E-SHD-009`, "sealed-shard immutability violation: refusing to
   overwrite an existing seal at `<path>`") and applies NEITHER the seal NOR any of the roll's other
   steps (no truncate, no index publish) for this attempt; the pre-existing destination file is left
   completely untouched. **Why this is defense-in-depth, not the primary fix:** the primary,
   ROOT-CAUSE fix is Postcondition 7 catch point (i)'s corrected self-heal-first sequencing (above),
   which prevents `next_seal_seq` from ever being computed against an unreconciled index in the first
   place — under that fix, a well-formed dispatch should never attempt to publish at a colliding
   `seq`. This write-once guard is the SECOND, independent layer: it converts any residual
   seq-collision — whether from a self-heal reconciliation bug, an external actor placing a
   same-named file on disk (analogous to Invariant 9's external-anomaly framing for a 0-byte
   candidate), or a future code path this BC has not yet anticipated — into a loud, actionable,
   fail-safe error rather than a silent overwrite of durably-sealed history. This is consistent with
   Invariant 1's "no version of this check may silently proceed past a condition it cannot verify
   safe" principle, extended to the seal-publish step itself. **CODE CHANGE ROUTED (→ implementer):**
   `publish_sealed_shard` gains an existence-check-before-rename guard — an atomic exclusive-create
   primitive is preferred where the platform supports it, to avoid a TOCTOU race between the check
   and the rename (e.g. staging the destination check via `std::fs::OpenOptions::new().create_new(true)`
   or an equivalent platform-appropriate exclusive-rename primitive); the exact primitive is an
   implementation detail this BC does not mandate, but the OBSERVABLE guarantee — no overwrite, ever
   — is mandatory. **TEST ROUTED (→ test-writer):** a fixture that pre-creates a file at the
   destination `seq` path before a roll (prospective or retroactive) attempts to publish at that same
   `seq`, asserting `HookResult::Error`/`E-SHD-009` is returned, the pre-existing file's content is
   BYTE-IDENTICAL after the attempt (never overwritten), and no truncate or index-publish occurs. See
   EC-024 and its matching Canonical Test Vector.
...
4. **Retry-instruction wording is a single, fixed template PER OVER-CAP CASE — never divergent per
   ...
   **ADJUDICATED (cluster-2 LOCAL adversary pass-3, F-C2-P3-001, MAJOR) — this invariant is scoped
   ...
   DISTINCT, sanctioned case, not a violation of "single fixed template."** Two, and only two,
   templates exist, each fixed and tool-name-invariant WITHIN its own case:
   ...
   - **Case B — over-cap-without-content** (Postcondition 1's `Ok(None)` empty-canonical
     short-circuit fires): the distinct `build_empty_roll_retry_block_reason` template
     (Postcondition 2's "Empty-canonical retry template" clause), likewise identical regardless of
     tool name.
   ...
   Selection between the two templates is a pure function of the Postcondition 1 code path taken
   (`Ok(None)` short-circuit vs. the full four-step sequence) — never randomized, never combined,
   never a third ad-hoc wording. This invariant is VIOLATED only if (i) either template varies by
   tool name within its own case, (ii) a third distinct wording appears for either case, or (iii)
   the two templates' selection logic depends on anything other than which Postcondition 1 code
   path executed.
...
| EC-024 (NEW, cluster-2 LOCAL adversary pass-4, F-C2-P4-002) | `publish_sealed_shard` is invoked (prospectively or retroactively) with a target `seq` path that ALREADY exists on disk (e.g. a residual self-heal gap, or an external actor placing a same-named file) | `publish_sealed_shard` refuses to perform the `rename`, returns `HookResult::Error` (NEW `E-SHD-009`, "sealed-shard immutability violation"), and applies NEITHER the seal nor any subsequent step (no truncate, no index publish) for this attempt; the pre-existing file at that path is left byte-identical, untouched (Postcondition 8) |
...
| **NEW (cluster-2 LOCAL adversary pass-4, F-C2-P4-002, EC-024).** A roll (prospective or retroactive) computes `next_seal_seq=3`, but `decision-log.0003.md` ALREADY exists on disk (e.g. a residual self-heal gap not covered by the EC-023 fix, or an external actor) holding unrelated content | `publish_sealed_shard` detects the destination already exists BEFORE renaming, refuses to overwrite, and returns `HookResult::Error` (`E-SHD-009`, "sealed-shard immutability violation: refusing to overwrite an existing seal at decision-log.0003.md"); `decision-log.0003.md`'s pre-existing content is verified byte-identical after the attempt (never overwritten); no truncate, no index publish for this attempt | error |
...
- `crates/factory-dispatcher/src/shard_manager.rs` `execute_roll` (NEW, cluster-2 LOCAL adversary pass-3, F-C2-P3-001) — the `Ok(None)` empty-canonical short-circuit (Postcondition 1) and its distinct `build_empty_roll_retry_block_reason` message-construction function (Postcondition 2's "Empty-canonical retry template"), invoked from the SAME trigger-fire branch that constructs the unified rotate-and-retry template, selecting between the two per Invariant 4's adjudicated two-case scoping. **CORRECTED (cluster-2 LOCAL adversary pass-4, F-C2-P4-003) — `build_empty_roll_retry_block_reason` takes a THIRD parameter, `payload_len_bytes: usize`**, alongside `artifact_stem` and `shard_cap_bytes`, so the message can interpolate `<N>` (Postcondition 2's Empty-canonical retry template, EC-021). **CORRECTED (F-C2-P4-004) — `execute_roll`'s canonical-file read (step (a)) and its sealed-shard write (step (b)) are BYTE-LEVEL** (`std::fs::read`/`std::fs::write`), never a UTF-8-fallible `read_to_string`/`&str` path.
...
- `crates/factory-dispatcher/src/shard_manager.rs` `publish_sealed_shard` (CORRECTED, cluster-2 LOCAL adversary pass-4, F-C2-P4-002, MINOR) — gains a write-once existence-check-before-rename guard (an atomic exclusive-create primitive preferred to avoid a TOCTOU race); refuses to overwrite an existing destination and returns `HookResult::Error` (NEW `E-SHD-009`) on collision, applying neither the seal nor any subsequent roll step (Postcondition 8, EC-024)
